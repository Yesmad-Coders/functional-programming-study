...
const reduce = (predicate, array, initValue) => {
...
console.log(reduce((acc, item) => acc + item, [1, 2, 3], 9)); // 15
...
const curriedMap = (predicate) => (array) => {
  const result = [];
  for (let i = 0; i < array.length; i++) {
    result.push(predicate(array[i]));
  }
...
const curriedFilter = (predicate) => (array) => {
  const result = [];
  for (let i = 0; i < array.length; i++) {
    if (predicate(array[i])) {
      result.push(array[i]);
...
const pipe = (f1, f2, f3) => (list) =>
  f3 ? f3(f2(f1(list))) : f2 ? f2(f1(list)) : f1 ? f1(list) : false; // 함수 순차적으로 실행할 하나의 함수 내놓기;
